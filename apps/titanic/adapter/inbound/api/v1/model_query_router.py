...
model_query_router = APIRouter(prefix="/titanic/model", tags=["model-query"])
...
@model_query_router.get("/info")
async def get_model_info(request: Request):
    logger.info("[QUERY] 모델 정보 조회 요청")
    use_case = _get_use_case(request)
    model = use_case.get_model_name()
    accuracy = use_case.get_accuracy()
    logger.info("[QUERY] 모델 정보 조회 완료 — model: %s, accuracy: %.4f", model, accuracy)
    return {"model": model, "accuracy": accuracy}


@model_query_router.get("/tree")
async def get_tree(request: Request):
    logger.info("[QUERY] 의사결정 트리 조회 요청")
    use_case = _get_use_case(request)
